...
## Goal
实现精灵蛋窝中心与蛋管理中心的双向联动：支持从蛋窝卡片一键产蛋到蛋管理中心（自动映射父母性格、三围、最低进化形态精灵名，现蛋数量加 1 且状态置为“有现蛋”）；反之，当精灵蛋在管理中心被删除时，系统自动追溯对应蛋窝卡片现蛋数减 1，并在归零时变更为“已撤窝”状态。另外，优化手动登记蛋的默认三围为生命/物攻/速度，以及选择大粗、大婉、单大块头时在未达极限时不显示“普通体型”标签。
...
- [决策]: 删除精灵蛋时，若其包含 `fromNestId` 属性，则使对应 ID 的蛋窝现蛋数减 1，降至 0 时将窝状态重置为“已撤窝”。
