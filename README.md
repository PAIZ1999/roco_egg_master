...
### 2. 精灵一键导入步骤
1. **登录游戏**：
   - 打开您的洛克王国客户端并登录游戏角色，在游戏内打开“宠物箱/仓库”让网络封包完成初始化加载。
2. **在管理系统中发起导入**：
   - 点击系统右上角的 **「导入游戏数据」**（或在父母本仓库界面点击“从游戏导入数据”）。
   - **导入方式选择**：
     *   **直连 SQLite 数据库导入**：系统会自动扫描本地抓取角色列表，下拉选择需要导入的角色昵称 UID，点击“从 SQLite 导入”即可一键秒级加载。
     *   **直连助手 API 导入**：保持抓取助手在后台运行，点击“直连助手 API 获取”，程序会通过安全的代理层获取全部宠物箱及仓库的个体值、身高体重、性格和存放位置信息。
     *   **文本粘贴导入**：可直接在文本框中粘贴助手抓取导出的精灵 JSON 数据，点击“解析 JSON”导入。
3. **数据清洗与排重**：
   - 导入后，系统会运用 $O(1)$ 复杂度哈希排重算法，智能将与您在父母本仓储中已存在的数据行进行对比，自动高亮并剔除重复精灵，勾选确认后一键无缝写入父母本仓库。
...
- **Windows 便携版 EXE 打包**（一键生成自带 `001.ico` 图标的绿色便携版程序）：
  ...
  ```
  打包成功后，可在根目录下的 `dist-electron/` 目录中找到 **`洛克王国孵蛋数据管理系统 4.2.0.exe`**。
