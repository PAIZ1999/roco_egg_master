...
- **🃏 精美父母本卡片**：为每只父精灵或母精灵创建独立管理卡片，卡片内含：
  ...
  - **📏 标准身高体重参考**：引入全图鉴数据库（`全图鉴.json`），在系别图标下方自动展示该精灵的标准身高（🔩 Ruler 图标）与体重范围（⚖️ Weight 图标），支持多形态差异化匹配（如鸭吉吉的蓬松形态与紧实形态体重区间各自对应）。
  - **牌子 / 身高 / 体重 / 性格 / 三围**：全面覆盖孵蛋所需核心属性，支持实时编辑与持久化存储。
  - **配组勾选**：左上角配组复选框，可快速标记该精灵是否纳入当前配组计划。
- **🤖 智能繁育配对联动**：
  - 在智能繁育配对模块中，符合配对条件的配对卡片左上角会渲染**绿色勾勾**选中标记。
  - 点击卡片即可切换选中/取消状态，支持**单独选中某张卡片**后一键导入到父母本，也可批量全部导入。
  - 导入后自动解析蛋组、性格、三围等信息并填充到父母本卡片。
...
- **Windows 便携版 EXE 打包**（一键生成自带 `001.ico` 图标的绿色便携版程序）：
  ...
  ```
    打包成功后，可在根目录下的 `dist-electron/` 目录中找到 **`洛克王国孵蛋数据管理系统 4.1.0.exe`**。
...
#### 🔄 卡片数据复制粘贴与智能映射 (Ctrl+C / Ctrl+V)
- **卡片级便捷操作**：在蛋窝中心、父母本仓储和精灵蛋管理三个 Tab 下，支持点击选中（或鼠标 Hover）任意卡片，按 `Ctrl+C` 即可复制数据；按 `Ctrl+V` 可直接粘贴覆盖至目标卡片。
- **高保真独立隔离**：修正了复制粘贴中的浅拷贝指针引用问题。在数据层层面对复制源和目标进行深拷贝，并且强制剔除了 `id` 字段。每一张被粘贴出来的卡片均拥有独立的唯一 ID 标识与独立的内存空间，可单独选择与修改，不与源卡片产生任何联动。
- **智能跨类型字段转换**：
  - **蛋窝 ➔ 父母本**：粘贴至父本/母本时，自动剥离出对应性别一方的性格与三围；在父母本空白处粘贴时默认建母本。
  - **父母本 ➔ 蛋窝**：父本仅覆盖蛋窝的父方数据，母本覆盖母方及精灵主体名。
  - **精灵蛋 ➔ 父母本**：将精灵蛋的父母数据自动反向填充至父本(♂)或母本(♀)。
  - **精灵蛋 ➔ 蛋窝**：品种名自动降级转换为进化链的最低阶形态（例如 `翠顶夫人` ➔ `乖乖鹄`）。
- **无目标粘贴自动新建**：若当前未 Hover 或选中任意卡片，在页面空白处直接按 `Ctrl+V` 时，系统会判定当前的活动 Tab，在列表尾部（或精灵蛋前端）自动创建并生成对应类型的全新卡片并粘贴数据。
- **空白点击清除选中**：点击页面空白区域（非卡片及按钮控件）会自动清除当前的卡片高亮选中，界面更加清爽。
- **输入框焦点放行保护**：在输入框、下拉菜单等可文本编辑状态下，自动释放卡片快捷键拦截，保留系统原生的文本 Ctrl+C/V 复制粘贴。

#### 💾 自动存盘路径根目录优化
- **程序根目录直存**：去除了原先的多余 `data/` 子文件夹。程序启动时默认在运行的 EXE 或开发环境同级根目录下直接读写 `roco_egg_data.json`，让玩家寻找、备份和复制数据文件更加直观、便利。
- **防写受限降级**：继续保留了自动降级处理。在 `C:\Program Files` 等无写入权限的目录运行时，将自动降级至用户系统 AppData 漫游目录（`AppData\Roaming\RocoIncubatorTable\roco_egg_data.json`）读写，保障数据安全。
