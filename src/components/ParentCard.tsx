...
const typeColorMap: Record<string, string> = {
  ...
  "幽": "bg-violet-50 dark:bg-violet-950/20 text-violet-655 dark:text-violet-305 border-violet-200 dark:border-violet-900/30",
  ...
  "毒": "bg-purple-50 dark:bg-purple-950/20 text-purple-650 dark:text-purple-305 border-purple-200 dark:border-purple-900/30",
  ...
  "电": "bg-yellow-50 dark:bg-yellow-950/10 text-yellow-505 dark:text-yellow-405 border-yellow-200 dark:border-yellow-900/30",
...
export const ParentCard = React.memo(function ParentCard({
  ...
  const getStatusBadge = () => {
    ...
    if (isGiantBrand) {
      if (hVal >= thresholds.maxHeight && wVal >= thresholds.maxWeight) {
        badges.push(
          <span key="limit-giant" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-300 border border-rose-200/60 dark:border-rose-900/30 shadow-3xs select-none shrink-0 whitespace-nowrap">
      ...
      if (hVal <= thresholds.minHeight && wVal <= thresholds.minWeight) {
        badges.push(
          <span key="limit-tiny" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-indigo-50 dark:bg-indigo-950/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200/60 dark:border-indigo-900/30 shadow-3xs select-none shrink-0 whitespace-nowrap">
      ...
      if (hVal >= thresholds.maxHeight && wVal >= thresholds.giantWeightLine) {
        badges.push(
          <span key="giant-ok" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-900/30 shadow-3xs select-none shrink-0 whitespace-nowrap">
        ...
        badges.push(
          <span key="tiny-ok" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-sky-50 dark:bg-sky-950/20 text-sky-700 dark:text-sky-300 border border-sky-200/60 dark:border-sky-900/30 shadow-3xs select-none shrink-0 whitespace-nowrap">
        ...
        if (x <= y) {
          ...
          if (wVal < thresholds.giantWeightLine && x <= maxDiff) {
            badges.push(
              <span key="near-giant" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-amber-50 dark:bg-amber-950/10 text-amber-755 dark:text-amber-300 border border-amber-200/60 dark:border-amber-900/30 shadow-3xs select-none shrink-0 animate-pulse whitespace-nowrap" style={{ animationDuration: "2s" }}>
          ...
          if (wVal > thresholds.tinyWeightLine && y <= maxDiff) {
            badges.push(
              <span key="near-tiny" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-purple-50 dark:bg-purple-950/15 text-purple-700 dark:text-purple-300 border border-purple-200/60 dark:border-purple-900/35 shadow-3xs select-none shrink-0 animate-pulse whitespace-nowrap" style={{ animationDuration: "2s" }}>
                <span className="relative flex w-1.5 h-1.5">
    ...
    if (!hasVoiceBrand && parent.voice !== undefined && parent.voice !== null) {
      ...
      if (v <= -96) {
        badges.push(
          <span key="voice-coarse" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-300 border border-amber-200/60 dark:border-amber-900/30 shadow-3xs select-none shrink-0 whitespace-nowrap">
            🎤 粗嗓门 ({v})
          </span>
        );
      } else if (v >= 96) {
        badges.push(
          <span key="voice-sweet" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-purple-50 dark:bg-purple-950/20 text-purple-655 dark:text-purple-300 border border-purple-200/60 dark:border-purple-900/30 shadow-3xs select-none shrink-0 whitespace-nowrap">
            🎤 婉转声 ({v})
          </span>
        );
      } else if (v <= -90 && v >= -95) {
        badges.push(
          <span key="near-coarse" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-amber-50 dark:bg-amber-950/10 text-amber-700 dark:text-amber-350 border border-amber-250/50 dark:border-amber-900/30 shadow-3xs select-none shrink-0 animate-pulse whitespace-nowrap" style={{ animationDuration: "2.5s" }}>
            🎤 临近粗嗓 ({v})
          </span>
        );
      } else if (v >= 90 && v <= 95) {
        badges.push(
          <span key="near-sweet" className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-purple-50 dark:bg-purple-950/10 text-purple-700 dark:text-purple-355 border border-purple-250/50 dark:border-purple-900/30 shadow-3xs select-none shrink-0 animate-pulse whitespace-nowrap" style={{ animationDuration: "2.5s" }}>
            🎤 临近婉转 ({v})
          </span>
        );
      }
  ...
  return (
    ...
    >
      {/* Left Column: Avatar, Checkbox, Delete, Sprite Data Indicators (guideSize/thresholds), getStatusBadge() and Location */}
      <div className="w-full sm:col-span-5 flex flex-row sm:flex-col items-center sm:border-r sm:border-slate-100 dark:sm:border-slate-800 pr-0 sm:pr-2.5 relative min-h-0 gap-3 sm:gap-1.5">
        ...
        <div className="w-24 h-24 sm:w-full sm:h-auto sm:aspect-square rounded-xl border border-slate-150 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/35 flex items-center justify-center shadow-inner relative group/avatar overflow-hidden shrink-0">
          ...
          {availableSprites.length > 1 && (
            <div 
              className="absolute bottom-0.5 left-0.5 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xs px-1.5 py-0.5 rounded shadow-2xs z-40 border border-slate-200/85 dark:border-slate-700 flex items-center hover:bg-white dark:hover:bg-slate-755 transition-colors duration-150"
        ...
        <div className="flex flex-col items-start sm:items-center gap-1 sm:gap-1.5 flex-1 min-w-0 pr-10 sm:pr-0 w-full">
          ...
          {/* 精灵物理数据标准区间与及格线 */}
          {guideSize && (
            <div className="w-full mt-2 bg-slate-50/90 dark:bg-slate-950/30 border border-slate-100/80 dark:border-slate-800/80 p-2 rounded-lg text-[10px] text-slate-500 dark:text-slate-400 select-none shadow-3xs flex flex-col gap-1 w-full text-left">
              <div className="flex items-center gap-1.5 font-bold text-slate-705 dark:text-slate-300">
                <Ruler className="w-3.5 h-3.5 text-slate-400 dark:text-slate-505 shrink-0" />
                <span>{guideSize.height} m</span>
              </div>
              <div className="flex items-center gap-1.5 font-bold text-slate-705 dark:text-slate-300">
                <Weight className="w-3.5 h-3.5 text-slate-400 dark:text-slate-505 shrink-0" />
                <span>{guideSize.weight} kg</span>
              </div>
              {thresholds && (
                <>
                  <div className="border-t border-slate-200/50 dark:border-slate-800/80 my-0.5" />
                  <div className="text-[9px] text-slate-400 dark:text-slate-500 flex flex-col gap-0.5 font-semibold">
                    <div>大及格: <span className="text-amber-600 dark:text-amber-400 font-extrabold">≥{thresholds.giantWeightLine}kg</span></div>
                    <div>小及格: <span className="text-sky-655 dark:text-sky-400 font-extrabold">≤{thresholds.tinyWeightLine}kg</span></div>
                  </div>
                </>
              )}
            </div>
          ...
          {/* 呼吸灯标签位置移动到精灵数据下方 (getStatusBadge) */}
          {getStatusBadge() && (
            <div className="flex justify-start w-full mt-1.5 shrink-0 select-none">
              {getStatusBadge()}
            </div>
          )}

          {/* 📍 游戏内仓库位置定位 Badge (排在精灵数据/呼吸状态灯的下面) */}
          {parent.position && parent.position !== "-" && (
            <div className="flex items-center justify-between gap-1.5 bg-indigo-50/60 dark:bg-indigo-950/20 p-1.5 rounded-lg border border-indigo-100/40 dark:border-indigo-900/30 shrink-0 select-none w-full mt-1.5">
              <span className="text-[9px] font-bold text-indigo-750 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200/50 dark:border-indigo-900/30 px-1.5 py-0.5 rounded flex items-center gap-0.5 shrink-0">
                <span>📍</span>游戏位置
              </span>
              <span className="text-[9px] font-extrabold text-indigo-650 dark:text-indigo-350 pr-1 truncate max-w-[110px]" title={parent.position}>
      ...
      {/* Right Column: Core Settings, Nature, Stats badges, and Egg Groups */}
      <div className="w-full sm:col-span-7 flex flex-col justify-between gap-2.5 border-t sm:border-t-0 border-slate-100 dark:border-slate-800 pt-2.5 sm:pt-0">
        ...
        {/* Core Profile: Brand, Height, Weight, Voice (Vertical column. Inputs set to flex-1 min-w-0 to prevent occlusion) */}
        <div className="flex flex-col gap-2.5 bg-slate-50/70 dark:bg-slate-900/40 p-2 rounded-lg border border-slate-100/60 dark:border-slate-800 shrink-0">
          {/* Brand */}
          <div className="flex items-center justify-between gap-3">
            <span className="text-[10px] font-bold text-slate-450 dark:text-slate-400 w-12 shrink-0 select-none text-left">牌子</span>
            <div className="flex-1 relative">
              <select
                value={parent.brand}
                onChange={(e) => handleUpdateParentBrand(parent.id, e.target.value)}
                className={`appearance-none text-xs font-bold text-center border rounded-md py-0.5 px-2 w-full cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-300 dark:focus:ring-indigo-950 transition-colors h-7 ${getBrandStyle(
                  parent.brand
                )}`}
              >
          ...
          {/* Height (Ruler) */}
          <div className="flex items-center justify-between gap-3">
            <span className="text-[10px] font-bold text-slate-450 dark:text-slate-400 w-12 shrink-0 select-none text-left">身高</span>
            <div className="flex-1 relative flex items-center rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus-within:border-indigo-400 dark:focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100 dark:focus-within:ring-indigo-950/40 transition-all shadow-3xs overflow-hidden h-7">
              <div className="pl-2 flex items-center text-slate-400 dark:text-slate-500 shrink-0">
              ...
              <input
                ...
                placeholder="输入身高"
                className="flex-1 min-w-0 w-0 text-xs font-bold pl-1.5 text-slate-800 dark:text-slate-100 bg-transparent py-0.5 border-none focus:outline-none focus:ring-0 placeholder:text-slate-400 dark:placeholder:text-slate-500"
              />
              <span className="text-[9px] font-bold text-slate-400 pr-2 shrink-0">m</span>
            </div>
          </div>

          {/* Weight (Weight/Scale) */}
          <div className="flex items-center justify-between gap-3">
            <span className="text-[10px] font-bold text-slate-450 dark:text-slate-400 w-12 shrink-0 select-none text-left">体重</span>
            <div className="flex-1 relative flex items-center rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus-within:border-indigo-400 dark:focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100 dark:focus-within:ring-indigo-950/40 transition-all shadow-3xs overflow-hidden h-7">
              <div className="pl-2 flex items-center text-slate-400 dark:text-slate-500 shrink-0">
              ...
              <input
                ...
                className="flex-1 min-w-0 w-0 text-xs font-bold pl-1.5 text-slate-800 dark:text-slate-100 bg-transparent py-0.5 border-none focus:outline-none focus:ring-0 placeholder:text-slate-400 dark:placeholder:text-slate-500"
              />
              <span className="text-[9px] font-bold text-slate-400 pr-2 shrink-0">kg</span>
            </div>
          </div>

          {/* Voice (🎤 Input) */}
          <div className="flex items-center justify-between gap-3">
            <span className="text-[10px] font-bold text-slate-450 dark:text-slate-400 w-12 shrink-0 select-none text-left">声音值</span>
            <div className="flex-1 relative flex items-center rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus-within:border-indigo-400 dark:focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100 dark:focus-within:ring-indigo-950/40 transition-all shadow-3xs overflow-hidden h-7">
              <div className="pl-2 flex items-center text-slate-400 dark:text-slate-500 shrink-0">
        ...
        {/* STATS Section (移回右侧，性格下方) */}
        <div className="flex flex-col gap-1.5 bg-slate-50/70 dark:bg-slate-900/40 p-1.5 rounded-lg border border-slate-100/60 dark:border-slate-800 shrink-0">
        ...
        {parent.groups && parent.groups.length > 0 && (
          <div className="flex items-center justify-between gap-1.5 bg-slate-50/70 dark:bg-slate-900/40 p-1.5 rounded-lg border border-slate-100/60 dark:border-slate-800 shrink-0">
            <span className="text-[10px] font-bold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-900/30 px-1.5 py-0.5 rounded select-none shrink-0">
              所属组别
            </span>
            <div className="flex flex-wrap gap-1 items-center justify-end flex-1">
              {parent.groups.map((grp) => (
                <span
                  key={grp}
                  className={`text-[9px] font-bold py-0.5 px-1.5 rounded-full border shadow-3xs select-none ${getEggGroupStyle(
                    grp
                  )}`}
                >
                  {grp}
